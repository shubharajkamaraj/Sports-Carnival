...
// IMPORTANT:
// Prisma enum is GOAL / MISSED.
// Do NOT use SCORED here.
type FootballPenaltyResult =
  | "GOAL"
  | "MISSED";
...
export default function FootballMatchPage() {
  ...
  const [match, setMatch] =
    useState<FootballMatch | null>(null);
  ...
  // Values are only GOAL or MISSED.
  ...
  async function addEvent() {
    ...
    try {
      ...
      const response =
        await fetch(
          `/api/matches/${matchId}/football/event`,
          {
            method: "POST",
      ...
      if (
        ...
        toast.success(
          "Football event added successfully."
        );
      }
  ...
  async function deleteFootballEvent(
    eventId: number
  ) {
    if (!match) {
      return;
    ...
    try {
      ...
      const response =
        await fetch(
          `/api/matches/${matchId}/football/event/${eventId}`,
          {
            method: "DELETE",
          }
        );
  ...
  async function undoLastFootballEvent() {
    if (!match) {
      return;
    }
    ...
    try {
      ...
      const response =
        await fetch(
          `/api/matches/${matchId}/football/undo`,
          {
            method: "POST",
          }
        );
  ...
  async function updateMatchStatus(
    ...
    try {
      ...
      const response =
        await fetch(
          `/api/matches/${matchId}/football/status`,
          {
            method: "POST",
      ...
      console.error(
        "UPDATE FOOTBALL MATCH STATUS ERROR:",
        error
      );
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-6">
              ...
              <select
                ...
                onChange={(e) =>
                  handleEventTypeChange(
                    e.target.value as FootballEventType
                  )
                }
                disabled={
                  saving ||
            ...
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Only players from the selected team are shown.
              </p>
            ...
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Enter the minute when the event happened.
              </p>
            ...
            <button
              ...
              className="w-full bg-black text-white rounded-lg py-3 font-semibold disabled:cursor-not-allowed disabled:opacity-50"
            >
              {saving
                ? "Adding..."
                : "Add Event"}
            ...
            {match.footballEvents
              ...
              !matchCompleted && (
                <button
                  type="button"
                  onClick={
                    undoLastFootballEvent
                  }
                  disabled={
                    eventActionLoading
                  }
                  className="mb-5 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <Undo2
                    size={16}
                  />
